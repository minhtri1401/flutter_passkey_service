...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): AuthGenerateOptionResponseData {
      ...
      return AuthGenerateOptionResponseData(rpId, challenge, allowCredentials, timeout, userVerification, hints, extensions)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
data class AuthPasskeyExtensionResult (
...
)
 {
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is AuthPasskeyExtensionResult) {
      return false
    }
    if (this === other) {
      return true
